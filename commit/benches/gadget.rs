...
fn small_params() -> GadgetParams {
    GadgetParams {
        ...
        base: 4,
        digits: 3,
        security_bits: 0, // TBD: Gadget security calibration is not finalized yet.
    }
}
...
fn bench_gadget(c: &mut Criterion) {
    ...
    let small_scheme = GadgetCommitmentScheme::<F17>::setup(&mut rng, &small_params()).unwrap();
    let small_message = RingVec::new(vec![F17::from_u64(5), F17::from_u64(6)]);
    let (small_commitment, small_opening) = small_scheme.commit(&small_message, &mut rng).unwrap();

    let rq_scheme = GadgetCommitmentScheme::<Rq23Np8>::setup(&mut rng, &rq23_np8_params()).unwrap();
    ...
    c.bench_function("gadget/commit/f17", |b| {
        b.iter_batched(
            grid_std::test_rng,
    ...
    c.bench_function("gadget/verify/f17", |b| {
        b.iter(|| {
            small_scheme
    ...
    c.bench_function("gadget/commit/rq23_np8", |b| {
        b.iter_batched(
            grid_std::test_rng,
    ...
    c.bench_function("gadget/verify/rq23_np8", |b| {
        b.iter(|| {
            rq_scheme
