...
fn bench_ajtai(c: &mut Criterion) {
    ...
    let small_scheme = AjtaiCommitmentScheme::<F17>::setup(&mut rng, &small_params()).unwrap();
    ...
    let rq_scheme = AjtaiCommitmentScheme::<Rq23Np8>::setup(&mut rng, &rq23_np8_params()).unwrap();
    ...
    c.bench_function("ajtai/commit/f17", |b| {
        b.iter_batched(
            grid_std::test_rng,
    ...
    c.bench_function("ajtai/verify/f17", |b| {
        b.iter(|| {
            small_scheme
    ...
    c.bench_function("ajtai/commit/rq23_np8", |b| {
        b.iter_batched(
            grid_std::test_rng,
    ...
    c.bench_function("ajtai/verify/rq23_np8", |b| {
        b.iter(|| {
            rq_scheme
